...
public class PromptBuilder {
	...
	public static String buildPrompt(String userInput, List<PlaceDTO> places) {
		...
		for (int i = 0; i < places.size(); i++) {
			...
			sb.append(String.format("%d. %s (%s, %s) - %s / %s\n", i + 1, p.getName(), p.getType(), p.getRegion(),
					p.getDescription(), p.getReason(), p.getImageUrl()));
		}
	...
	public static String buildPrompt2(List<PlaceDTO> results, String dateStr) {
		...
		sb.append(
				"- restaurant, cafe, bar, bakery, tourist_attraction, museum, zoo, amusement_park, aquarium, shopping_mall, clothing_store, park, natural_feature\n");
		...
		sb.append(
				"당신의 두번째 역할은 리스트에서 삭제조건에 해당하는 장소를 삭제한 뒤, 리스트에 들어있는 장소의 정보 중 null 값을 가진 description과 reason의 내용을 생성하고 넣어주는 것입니다.\n");
		sb.append("📌 수정 조건 \n");
		...
		sb.append(
				"당신의 세번째 역할은 리스트에서 첫번째와 두번째 역할을 수행한 뒤, 리스트에 들어있는 장소의 정보 중 name 값이 한글이 아닌 경우에 대해서만 해당 값을 한글로 수정하는 것입니다.\n");
		sb.append("📌 수정 조건 \n");
		...
		for (int i = 0; i < results.size(); i++) {
			...
			sb.append(String.format("%d. %s (%s, %s) - %s / %s / %s / %s / %s\n", i + 1, p.getName(), p.getType(),
					p.getRegion(), p.getDescription(), p.getReason(), p.getLatitude(), p.getLongitude(),
					p.getImageUrl()));
